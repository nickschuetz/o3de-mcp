...
- Editor tools use raw TCP sockets to send `pyRunScript` commands. Host/port are configurable via `O3DE_EDITOR_HOST` and `O3DE_EDITOR_PORT` env vars (default: `127.0.0.1:4600`), and the per-command timeout via `O3DE_EDITOR_TIMEOUT` (default: 10s). The scripts use the `azlmbr` namespace available inside the O3DE Editor Python environment.
